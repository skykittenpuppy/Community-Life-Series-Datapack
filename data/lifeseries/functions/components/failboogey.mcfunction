...
execute if score lastlife constants matches 1.. run scoreboard players set @a[tag=boogeyman] lives 1
...
execute if score limitedlife constants matches 1.. run tellraw @a[tag=boogeyman,scores={ticks=576001..1152000}] [{"text":"You failed to kill another player last session as the boogeyman. As punishment, you have dropped to the next colour. You have ","color":"gray"},{"text":"8 hours","color":"red"},{"text": "."}]
